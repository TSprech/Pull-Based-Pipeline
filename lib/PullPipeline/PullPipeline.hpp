...
struct pull_input : std::function<Output_Type()> { // These need to be strongly typed in order for the operators to overload correctly
...
struct pull_output : std::function<void(Input_Type... input)> { // Strongly typed
...
template <typename Output_Type, typename... Input_Type> // Output_Type is just what type the function returns, Input_Type is variadic for the same reason it is in pull_output, it will take in other functions
struct pull_filter : std::function<Output_Type(Input_Type... input)> { // Strongly typed
...
template <typename Output_Type, typename... Input_Type> // Output_Type is the type that is returned from the pull_input, Input_Type is the aggregate of all the functions leading up to the pull_output
void operator<<(pull_output<Input_Type...> output, pull_input<Output_Type> input){ // The left hand side takes in the lambda that will receive the processed value and the right hand is the function that generates the initial pipeline input
...
// This is the most complex part of the pipeline, this operator generates an output lambda that has a new parameter which is a pull_filter lambda
...
// So this lambda is returned as a new pull_output which will take in some number of arguments and will now execute output(filter(<argument to come later>))
// As this process is repeated, we end up with a single pull_output lambda which "contains" all the other filters in the order they were placed in, ready to execute as soon as an argument is given
// Eventually we reach a point where we have output(filter1(filter2(filter3(filter4(<still waiting on arguments> and then the pull_input lambda is reached, which at that point the other operator kicks in
// Keep in mind that output(filter1(filter2(filter3(filter4(<still waiting on arguments> has all been compressed into a single lambda so it is in effect a pull_output
// The input is given to the aggregate pull_output via output(input()); and the entire pipeline is executed
...
pull_output<Output_Type> operator<<(pull_output<Output_Type> output, pull_filter<Output_Type, Input_Type...> filter){
