...
namespace pipes {
   ...
   */
  template <typename Output_Type, typename... Input_Type>
  auto operator<<(PullOutput<Input_Type...> output, PullInput<Output_Type> input) {
    return [=]() { output(input()); };  // Call all lambdas starting with the input | eg: [simple] output_lambda(input_lambda())   [complex] output_lambda(filter_1(filter_2(filter_3(input_lambda()))));
  }
  ...
  PullOutput<Output_Type> operator<<(PullOutput<Output_Type> output, PullFilter<Output_Type, Input_Type...> filter) {
    return [=](auto input) {  // input is all the fittings that are yet to come
